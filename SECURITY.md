...
Hypershell Mobile Bridge crosses an authenticated remote-control boundary and is intentionally narrow.
...
- The private control API has an independent high-entropy bearer token and is intended to bind only to loopback or another equivalently private boundary.
...
Please do **not** publish suspected vulnerabilities, credentials, device identifiers, screenshots or exploit details in a public issue.

Use GitHub's **private vulnerability reporting / Security Advisory** flow for this repository when available. Public issues are appropriate for non-sensitive bugs and feature requests only.
